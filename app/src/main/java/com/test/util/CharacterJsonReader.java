...
public class CharacterJsonReader {
    ...
    public static String query(Context context,String word)
    {
        ...
        Cursor cursor1=sqLiteDatabase.query(table,columns,"学习次数 < ? ",new String[]{"3"},null,null,null,"20");
        while(cursor1.moveToNext()) {
            String content=cursor1.getString(cursor.getColumnIndex("常用字"))+cursor.getString(cursor.getColumnIndex("简体字"))+cursor.getString(cursor.getColumnIndex("学习次数"));
            SB.append(content);
        }
    ...
    public static SQLiteDatabase DBManage(Context mContext,String packname)
    {
        ...
        if (!new File(dbPath).exists()) {
            try {
                ...
                boolean newFile = new File(dbPath).createNewFile();
                try {
                    OutputStreamWriter out = new OutputStreamWriter(new FileOutputStream(dbPath));
                    InputStreamReader in = new InputStreamReader(mContext.getAssets().open("test.db"));
